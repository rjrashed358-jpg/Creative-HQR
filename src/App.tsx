      ...
      void main() {
        ...
        for(int m=0; m<AA; m++) {
          for(int n=0; n<AA; n++) {
            ...
            vec3 col = renderPixel(gl_FragCoord.xy + o);
            tot += col;
          }
        }
        tot /= float(AA*AA);
        #else
        tot = renderPixel(gl_FragCoord.xy);
        #endif
    ...
    <div className="fixed inset-0 bg-black overflow-hidden">
          ...
          >
            ...
            >
              ...
              <div className="space-y-4 pt-4">
                {/* Pause Toggle */}
                <div className="flex items-center justify-between" title="Toggles the automatic rotation of the fractal scene.">
                ...
                {/* Speed Control */}
                <div className="space-y-2" title="Controls the temporal speed of the scene's rotation and animation.">
                ...
                {/* Lighting Control */}
                <div className="space-y-2" title="Adjusts the intensity of the light source illuminating the fractal.">
                  <div className="flex justify-between text-[9px] font-mono uppercase tracking-tighter">
                ...
                {/* Zoom Control */}
                <div className="space-y-2" title="Controls the focal zoom level, bringing the camera closer or further away.">
                  <div className="flex justify-between text-[9px] font-mono uppercase tracking-tighter">
                ...
                {/* Proximity Control */}
                <div className="space-y-2" title="Adjusts the physical distance of the camera from the center of the fractal.">
                  <div className="flex justify-between text-[9px] font-mono uppercase tracking-tighter">
                ...
                {/* Wind Control */}
                <div className="space-y-2" title="Controls the speed and turbulence of the atmospheric mist.">
                  <div className="flex justify-between text-[9px] font-mono uppercase tracking-tighter">
                ...
                {/* Yaw Control */}
                <div className="space-y-2" title="Rotates the camera around the horizontal axis (left/right).">
                  <div className="flex justify-between text-[9px] font-mono uppercase tracking-tighter">
                ...
                {/* Pitch Control */}
                <div className="space-y-2" title="Rotates the camera around the vertical axis (up/down).">
                  <div className="flex justify-between text-[9px] font-mono uppercase tracking-tighter">
                ...
                {/* Color Mode Control */}
                <div className="space-y-2" title="Changes the primary color palette of the fractal's atmosphere and lighting.">
      ...
      <AnimatePresence>
        {audioStarted && (
          <motion.div
            ...
            className="fixed bottom-6 left-6 z-40 pointer-events-none flex flex-col gap-2 font-mono text-[10px] uppercase tracking-widest bg-black/20 backdrop-blur-md border border-white/5 p-4 rounded-xl shadow-2xl"
          >
            <div className="flex items-center gap-4">
              <span className="w-16 text-white/40">ZOOM</span>
              <span className="text-white/90">{zoom.toFixed(3)}x</span>
            </div>
            <div className="flex items-center gap-4">
              <span className="w-16 text-white/40">YAW</span>
              <span className="text-white/90">{(yaw * (180 / Math.PI)).toFixed(1)}°</span>
            </div>
            <div className="flex items-center gap-4">
              <span className="w-16 text-white/40">PITCH</span>
              <span className="text-white/90">{(pitch * (180 / Math.PI)).toFixed(1)}°</span>
            </div>
            <div className="flex items-center gap-4">
              <span className="w-16 text-white/40">PALETTE</span>
              <span className="text-white/90">{colorMode}</span>
            </div>
            <div className="flex items-center gap-4">
              <span className="w-16 text-white/40">SPEED</span>
              <span className="text-white/90">{speed.toFixed(2)}x</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {audioStarted && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 w-10 h-10 bg-black/40 backdrop-blur-xl border border-white/10 rounded-full flex items-center justify-center hover:bg-white/10 transition-all shadow-2xl group"
          title={isFullscreen ? "Exit Fullscreen" : "Enter Fullscreen"}
        >
          {isFullscreen ? (
            <Minimize className="w-4 h-4 text-white/60 group-hover:text-white/90" />
          ) : (
            <Maximize className="w-4 h-4 text-white/60 group-hover:text-white/90" />
          )}
        </button>
      )}
    </div>
